...
import { fetchAccountHomeDomain } from "./horizon.js";
...
export async function discoverDomain(
  ...
  try {
    ...
    // A [[CURRENCIES]] issuer is frequently a third party's account, not this
    // domain's own: an anchor supporting official Circle USDC cites Circle's
    // real issuer, exactly as SEP-1 requires, because it isn't issuing its own
    // asset. Surfacing that address under this domain's name would scan
    // Circle's global USDC settlement volume as if it belonged to one anchor —
    // confirmed live for stellar.moneygram.com, whose toml cites the same
    // issuer (GA5ZSEJY...KZVN) as at least one other anchor. ACCOUNTS=[]
    // entries need no such check; SEP-1 already defines those as the domain's
    // own. So only issuers are verified here, against the issuer account's own
    // home_domain read off the ledger.
    for (const account of parsed.issuers) {
      if (seen.has(account)) continue;
      const issuerHomeDomain = await fetchAccountHomeDomain(horizon, account, fetchImpl);
      if (!issuerHomeDomain || issuerHomeDomain.toLowerCase() !== domain.toLowerCase()) continue;
      seen.add(account);
