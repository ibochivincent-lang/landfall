...
/**
 * Reads an account's `home_domain` straight off the ledger.
 *
 * Used to confirm that a stellar.toml [[CURRENCIES]] issuer actually belongs
 * to the domain citing it — see `discoverDomain` in toml.ts for why that
 * check exists. Returns undefined on any failure (unset field, network error,
 * unknown account): the caller treats "can't confirm" the same as "doesn't
 * match", which is the safe default here.
 */
export async function fetchAccountHomeDomain(
  horizon: string,
  account: string,
  fetchImpl: typeof fetch = fetch,
): Promise<string | undefined> {
  try {
    const body = await getJson(`${horizon.replace(/\/$/, "")}/accounts/${account}`, fetchImpl);
    const homeDomain = body["home_domain"];
    return typeof homeDomain === "string" ? homeDomain : undefined;
  } catch {
    return undefined;
  }
}
