...
(() => {
  ...
  const aiSubmitBtn = $('#aiChatSubmit');
  ...
  if (aiForm) {
    aiForm.addEventListener('submit', async (e) => {
      ...
      // Add user message
      aiBody.innerHTML += '<div class="ai-message user">' + esc(query) + '</div>';
      aiInput.value = '';
      
      const thinkingId = 'think-' + Date.now();
      aiBody.innerHTML += '<div class="ai-message ai" id="' + thinkingId + '">Analyzing ledger data...</div>';
      ...
      try {
        ...
        const thinkEl = $('#' + thinkingId);
        
        if (!res.ok) {
          thinkEl.className = 'ai-message error';
          thinkEl.textContent = data.error || 'Failed to get a response from the AI.';
          return;
        }

        // Render AI explanation
        let html = '<div class="ai-sql-badge">SQL Executed</div><br>';
        html += '<strong>' + esc(data.explanation) + '</strong>';
        ...
        if (data.rows && data.rows.length > 0) {
          ...
          data.rows.forEach(r => {
            ...
            keys.forEach(k => {
              ...
              if (v !== null && v !== undefined) {
                if (k.includes('amount')) v = amount(v);
                else if (k.includes('at')) v = v.substring(0, 10);
                else if (typeof v === 'string' && v.length > 20 && k !== 'memo') v = short(v);
              }
              html += '<td>' + esc(v) + '</td>';
            });
            html += '</tr>';
          ...
          html += '<div class="ai-message ai"><em>No rows returned for this query.</em></div>';
        }
