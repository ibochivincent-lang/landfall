...
(() => {
  ...
  function renderAnchors(accounts, reliability = {}) {
    ...
    function renderStats(accounts, domain) {
      ...
      // The dashboard is a live view over the whole set; the permanent record
      // for one anchor lives at its own URL, which is the thing that can be
      // linked to, cited, or sent to someone doing diligence.
      const permalink = $('#anchorPermalink');
      if (permalink) {
        permalink.href = 'anchor/' + encodeURIComponent(domain) + '.html';
        permalink.textContent = 'Full record for ' + domain + ' →';
