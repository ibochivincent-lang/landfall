(() => {
  ...
  function render(accounts, meta) {
    ...
    // Finding banner: previously a hand-typed sentence ("6 of 13 … 12 August
    // 2026") that silently went stale and, worse, went false — a later scan
    // showed 5 of 13 dark with no anchor where every history-bearing account
    // was dark, while the banner kept asserting both. Computed fresh from the
    // same accounts array the rest of this function reads, every render.
    if ($('findingDate') && meta && meta.asOf) {
      const fd = new Date(meta.asOf);
      if (!isNaN(fd)) {
        $('findingDate').textContent = fd.toLocaleDateString('en-GB', {
          day: 'numeric', month: 'long', year: 'numeric',
        });
      }
    }
    if ($('findingBody')) {
      const byDomain = {};
      accounts.forEach(a => {
        const key = a.domain || 'unknown';
        (byDomain[key] = byDomain[key] || []).push(a);
      });
      let allDarkDomain = null;
      for (const [domain, accts] of Object.entries(byDomain)) {
        const withHistory = accts.filter(a => (Number(a.inbound || 0) + Number(a.outbound || 0)) > 0);
        if (withHistory.length > 0 && withHistory.every(a => (a.state || a.status) === 'dark')) {
          allDarkDomain = domain;
          break;
        }
      }
      let body = dark + ' of ' + total + ' anchor accounts have processed no on-chain settlement in over 30 days.';
      body += allDarkDomain
        ? ' Every account with payment history at ' + allDarkDomain + ' is dark.'
        : ' No single anchor currently has every history-bearing account dark.';
      body += ' Verified against stellar.expert. Every figure ships with its transaction hashes.';
      $('findingBody').textContent = body;
    }
  }
