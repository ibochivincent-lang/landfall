 ...
 */
import { GRADE_ORDER, LIQUIDITY_ORDER, gradeAtLeast, } from "./types.js";
 ...
 */
export function solveIntent(intent, candidates, midRate) {
    const solutions = [];
    const rejected = [];
    ...
    const byAmount = (a, b) => intent.basis === "send" ? (b.receive ?? 0) - (a.receive ?? 0) : (a.send ?? 0) - (b.send ?? 0);
    const better = (a, b) => {
