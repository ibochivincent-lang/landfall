...
const CAUTION_AT_OR_BELOW = ["D", "F", "U"];
export function buildPlan(input) {
    ...
    push({
        ...
        detail: "Read what the ledger shows about the anchor's own declared accounts — observed history, " +
    ...
    push({
        ...
        detail: `Sending ${from} requires a trustline to that asset's issuer. Most wallets do this automatically ` +
            "when needed; it is listed because it is a real on-chain operation with its own fee and reserve.",
    });
    push({
        id: "send-payment",
        actor: "wallet",
        title: solution.send !== null
        ...
        detail: "The destination address and memo come from the anchor's own SEP-24 response, never from Landfall — " +
    ...
    push({
        ...
        detail: "This is the only step that produces evidence the fiat leg completed. Nothing on-chain shows a bank " +
    ...
    return {
        ...
        steps,
        executionNote: "This is a plan, not an execution. Landfall holds no keys, no funds and no custody, and every step " +
