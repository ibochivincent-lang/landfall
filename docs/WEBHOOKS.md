...
| `refund.spike` | **Never** | Subscribable, with no producer anywhere in the codebase. Left in place rather than removed, because deleting it would silently drop a subscription someone chose. Do not rely on it |

A recovery — `slow` → `live` — deliberately fires nothing. An alert that
wakes you for good news trains you to ignore alerts.
